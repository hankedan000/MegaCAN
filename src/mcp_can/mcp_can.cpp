...
*********************************************************************************************************/
void MCP_CAN::mcp2515_read_canMsg( const INT8U buffer_sidh_addr, INT32U *id, INT8U *ext, INT8U *len, INT8U buf[])
{
    INT8U mcp_addr, ctrl;

    mcp_addr = buffer_sidh_addr;

    mcp2515_read_id( mcp_addr, ext, id );

    ctrl = mcp2515_readRegister( mcp_addr-1 );
    *len = mcp2515_readRegister( mcp_addr+4 );
    ...
    *len &= MCP_DLC_MASK;
    mcp2515_readRegisterS( mcp_addr+5, buf, *len );
}
...
** Descriptions:            Read message
*********************************************************************************************************/
void MCP_CAN::mcp2515_read_canMsg_fast( const INT8U rxbf, INT32U *id, INT8U *ext, INT8U *len, INT8U buf[])
{
    INT8U tbufdata[4];
    INT8U i = 0;
    SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    MCP2515_SELECT();
    if (rxbf == 0)
        spi_readwrite(MCP_READ_RX0);
    else
        spi_readwrite(MCP_READ_RX1);
...
{
    MCPCS_bit = digitalPinToBitMask(_CS);
    uint8_t port = digitalPinToPort(_CS);
    MCPCS_out = portOutputRegister(port);
    MCP2515_UNSELECT();
    pinMode(_CS, OUTPUT);
}
...
** Descriptions:            Send message
*********************************************************************************************************/
INT8U MCP_CAN::sendMsg(INT8U waitForSend)
{
...
*********************************************************************************************************/
INT8U MCP_CAN::sendMsgBuf(INT32U id, INT8U ext, INT8U len, INT8U *buf, INT8U waitForSend)
{
    INT8U res;
	
    setMsg(id, 0, ext, len, buf);
    res = sendMsg(waitForSend);
...
** Descriptions:            Send message to transmitt buffer
*********************************************************************************************************/
INT8U MCP_CAN::sendMsgBuf(INT32U id, INT8U len, INT8U *buf, INT8U waitForSend)
{
    INT8U ext = 0, rtr = 0;
    INT8U res;
...
#define FAST_RXBF_READ

/*********************************************************************************************************
** Function name:           readMsg
** Descriptions:            Read message
*********************************************************************************************************/
INT8U MCP_CAN::readMsg(INT32U *id, INT8U *ext, INT8U *len, INT8U buf[])
{
    INT8U stat, res;
    ...
    {
#ifdef FAST_RXBF_READ
        mcp2515_read_canMsg_fast(0, id, ext, len, buf);
#else
        mcp2515_read_canMsg( MCP_RXBUF_0, id, ext, len, buf);
    ...
    {
#ifdef FAST_RXBF_READ
        mcp2515_read_canMsg_fast(1, id, ext, len, buf);
#else
        mcp2515_read_canMsg( MCP_RXBUF_1, id, ext, len, buf);
...
{
    if(readMsg(id,ext,len,buf) == CAN_NOMSG)
        return CAN_NOMSG;
...
{
    if(readMsg(id,&m_nExtFlg,len,buf) == CAN_NOMSG)
        return CAN_NOMSG;
...
/*********************************************************************************************************
** Function name:           serviceErrors
** Descriptions:            Public function, Checks & clears errors. Calls correspond user handlers.
*********************************************************************************************************/
INT8U MCP_CAN::serviceErrors(void *varg, const MCP_ErrorHandlers *err_h)
{
    INT8U eflg = mcp2515_readRegister(MCP_EFLG);

    INT8U rec, tec;
    if ( eflg & (MCP_EFLG_EWARN | MCP_EFLG_RXWAR | MCP_EFLG_RXEP) )
        rec = errorCountRX();
    if ( eflg & (MCP_EFLG_EWARN | MCP_EFLG_TXWAR | MCP_EFLG_TXEP) )
        tec = errorCountTX();

    INT8U clear_flgs = 0;
    if ( eflg & MCP_EFLG_EWARN )
    {
        if (err_h->e_warn != NULL)
            err_h->e_warn(this,varg,rec,tec);
    }
    if ( eflg & MCP_EFLG_RXWAR )
    {
        if (err_h->rx_war != NULL)
            err_h->rx_war(this,varg,rec);
    }
    if ( eflg & MCP_EFLG_TXWAR )
    {
        if (err_h->tx_war != NULL)
            err_h->tx_war(this,varg,tec);
    }
    if ( eflg & MCP_EFLG_RXEP )
    {
        if (err_h->rx_ep != NULL)
            err_h->rx_ep(this,varg,rec);
    }
    if ( eflg & MCP_EFLG_TXEP )
    {
        if (err_h->tx_ep != NULL)
            err_h->tx_ep(this,varg,tec);
    }
    if ( eflg & MCP_EFLG_TXBO )
    {
        if (err_h->tx_bo != NULL)
            err_h->tx_bo(this,varg);
    }
    if ( eflg & MCP_EFLG_RX0OVR )
    {
        clear_flgs = clear_flgs | MCP_EFLG_RX0OVR;
        if (err_h->rx0_ovr != NULL)
            err_h->rx0_ovr(this,varg);
    }
    if ( eflg & MCP_EFLG_RX1OVR )
    {
        clear_flgs = clear_flgs | MCP_EFLG_RX1OVR;
        if (err_h->rx1_ovr != NULL)
            err_h->rx1_ovr(this,varg);
    }

    // reset buffer overflow flags
    if ( clear_flgs )
        mcp2515_modifyRegister(MCP_EFLG, clear_flgs, 0x00);
